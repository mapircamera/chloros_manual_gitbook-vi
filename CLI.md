...
1. Tải xuống và chạy**Chloros Installer.exe**2. Hoàn tất trình hướng dẫn cài đặt
...
Xử lý hình ảnh trong một thư mục có hiệu chuẩn.**Cú pháp:**```bash
...
Đăng nhập bằng thông tin đăng nhập Chloros+ của bạn để kích hoạt xử lý CLI.**Cú pháp:**```bash
...
{% endhint %}**đầu ra:**<figure><img src=".gitbook/assets/cli login_w.JPG" alt=""><figcaption></figcaption></figure>***

### `logout` - Clear Credentials

Xóa thông tin đăng nhập được lưu trữ và đăng xuất khỏi tài khoản của bạn.**Cú pháp:**```bash
chloros-cli logout
```**Ví dụ:**```powershell
chloros-cli logout
```**đầu ra:**```
...
Hiển thị giấy phép hiện tại và trạng thái xác thực.**Cú pháp:**```bash
chloros-cli status
```**Ví dụ:**```powershell
chloros-cli status
```**đầu ra:**```
...
Theo dõi tiến trình xuất Thread 4 trong hoặc sau khi xử lý.**Cú pháp:**```bash
...
Xem hoặc thay đổi ngôn ngữ giao diện CLI.**Cú pháp:**```bash
...
Thay đổi vị trí thư mục dự án mặc định (được chia sẻ với GUI).**Cú pháp:**```bash
...
Hiển thị vị trí thư mục dự án mặc định hiện tại.**Cú pháp:**```bash
...
Đặt lại thư mục dự án về vị trí mặc định.**Cú pháp:**```bash
...
| `--help`        | Lá cờ    | -             | Hiển thị thông tin trợ giúp và thoát                   |**Ví dụ với Tùy chọn chung:**```powershell
...
* **Tối đa: 16 công nhân song song**(để ổn định)**Cấp hệ thống:**| Loại hệ thống   | CPU        | ĐẬP      | Công nhân  | Hiệu suất     |
| ------------- | ---------- | -------- | -------- | --------------- |
...
{% endhint %}***## Khắc phục sự cố

### CLI Không tìm thấy**Lỗi:**```
...
```**Giải pháp:**1. Kiểm tra xem chương trình phụ trợ đã chạy chưa (đóng nó trước)
...
### Vấn đề về giấy phép / xác thực**Lỗi:**```
Chloros+ license required for CLI access
```**Giải pháp:**1. Xác minh bạn có đăng ký Chloros+ đang hoạt động
...
### Không tìm thấy hình ảnh nào**Lỗi:**```
No images found in the specified folder
```**Giải pháp:**1. Xác minh thư mục chứa các định dạng được hỗ trợ (.RAW, .TIF, .JPG)
...
### Xử lý gian hàng hoặc treo**Giải pháp:**1. Kiểm tra dung lượng đĩa trống (đảm bảo đủ cho đầu ra)
2. Đóng các ứng dụng khác để giải phóng bộ nhớ
3. Giảm số lượng hình ảnh (xử lý theo đợt)***

### Cổng đã được sử dụng**Lỗi:**```
Port 5000 is already in use
```**Giải pháp:**Chỉ định một cổng khác:
...
### Hỏi: Tôi có cần giấy phép cho CLI không?**A:**Yes! The CLI requires a paid**Chloros+ license**.
...
### Câu hỏi: Tôi có thể xử lý nhiều thư mục cùng một lúc không?**A:**Not directly in one command, but you can use scripting to process folders sequentially. See [Automation & Scripting](CLI.md#automation--scripting) section.***

### Câu hỏi: Làm cách nào để lưu đầu ra CLI vào tệp nhật ký?**PowerShell:**```powershell
chloros-cli process "C:\Datasets\Field_A" | Tee-Object -FilePath "processing.log"
```**Lô:**```batch
chloros-cli process "C:\Datasets\Field_A" > processing.log 2>&1
```***

### Hỏi: Điều gì xảy ra nếu tôi nhấn Ctrl+C trong khi xử lý?**A:**The CLI will:
...
### Câu hỏi: Tôi có thể tự động xử lý CLI không?**A:**Absolutely! The CLI is designed for automation. See [Automation & Scripting](CLI.md#automation--scripting) for PowerShell, Batch, and Python examples.***

### Hỏi: Làm cách nào để kiểm tra phiên bản CLI?**A:**```powershell
chloros-cli --version
```**đầu ra:**```
Chloros CLI 1.0.2
```***
...
* **Giá**: [https://cloud.mapir.camera/pricing](https://cloud.mapir.camera/pricing)***## Ví dụ hoàn chỉnh
